...
**Bibliothèque** : `assets/sfx/` — contient le **pack de démarrage livré** (`assets/sfx/starter/` :
`ui-click`, `pop`, `whoosh`, `riser`, `impact`, `beep`) plus tout MP3 que le créateur a ajouté (ses
propres sons, ou récupérés via l'API HeyGen — cf `design-system/sfx-sound-search.md`). **Se référer
aux fichiers réellement présents.** Les sons plus riches cités ci-dessous (camera shutter, felt pen,
frappe clavier…) ne sont là **que si** l'auth HeyGen a été faite ou que le créateur les a déposés —
sinon, se rabattre sur le pack de démarrage.
...
7. **Texte qui se tape** (ex. mot-clé du CTA) : `PC typing` **si disponible**, durée adaptée (quelques
   lettres → court ; longue phrase → doubler).
