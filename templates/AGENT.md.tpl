...
> **Charger un skill** :
> {{#CLAUDE_CODE}}Claude Code — utilise l'outil **Skill** (ou tape `/<nom>`).{{/CLAUDE_CODE}}{{#CODEX}}Codex — **ouvre le fichier** `.agents/skills/<nom>/SKILL.md` indiqué et relis-le.{{/CODEX}}
...
> skill `{{#CLAUDE_CODE}}/hyperframes{{/CLAUDE_CODE}}{{#CODEX}}hyperframes{{/CODEX}}` (son router) ni via un workflow générique du framework
